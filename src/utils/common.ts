...
export function throttle(fn, delay) {//throttle 与 debounce “合体”思路
...
export function readNodes(nodes, arr = [],num=0) { // 过滤得到树形结构selected为true的项
	for(const item of nodes) {
		...
		if(obj.num>1&&obj.component=="layout/index"){
			obj.component=nodes.filter(item=>item.component!=obj.component)[0].component;
		}
		arr.push(obj)
		if(item.children && item.children.length){
			obj.children=[];
			readNodes(item.children, obj.children,num+1)
		} 
	}
	return arr
}
